...
```bash
node scripts/self-test.mjs --epic epic-22
```

Config: `scripts/self-test-configs/epic-22.json`
...
- Admin components using `@payloadcms/ui` may need a mock Payload context provider — create `.storybook/decorators/PayloadMockProvider.tsx` if needed
